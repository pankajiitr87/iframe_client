...
function App() {
  ...
  const openFrameAndSendMessage = () => {
    ...
    const url = `https://https://bo-cilent-updated.vercel.app/?domain=${encodeURIComponent(domain)}&clientId=${encodeURIComponent(clientId)}&month=${encodeURIComponent(month)}&year=${encodeURIComponent(year)}`
    // Create the iframe
