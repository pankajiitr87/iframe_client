...
function App() {
  ...
  const openFrameAndSendMessage = () => {
    ...
    const url = `http://localhost:${port}/?domain=${encodeURIComponent(domain)}&clientId=${encodeURIComponent(clientId)}&month=${encodeURIComponent(month)}&year=${encodeURIComponent(year)}`
    // Create the iframe
